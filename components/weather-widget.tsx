...
export function WeatherWidget() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const randomTemp = Math.floor(Math.random() * 30) + 60; // Random temperature between 60-90°F
  ...
  // Get the appropriate weather icon based on the current weather
  ...
  // Get the appropriate gradient color based on the current weather
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex items-center space-x-2 rounded-full bg-gradient-to-r px-3 py-1.5",
        getWeatherColor()
      )}
    >
      {getWeatherIcon()}
